...
_CORE_MODULE_NAMES = {
    ...
    'reports', 'details', 'logs', 'topology',
}


def _discover_optional_mixins():
    """src/api/ 디렉터리에서 코어가 아닌 *.py 파일을 찾아 *Mixin 클래스를 자동 등록.

    반환: [(feature_id, MixinClass), ...]
        feature_id 는 모듈 파일명 (예: 'catalog' from 'catalog.py').
    ...
    for f in sorted(pkg_dir.glob('*.py')):
        stem = f.stem
        if stem in _CORE_MODULE_NAMES or stem.startswith('_'):
            continue
        try:
            mod = importlib.import_module(f'.{stem}', package=__package__)
            # 같은 파일에서 첫 번째 *Mixin 클래스 사용
            mixin_cls = next(
                (obj for name, obj in vars(mod).items()
                 if name.endswith('Mixin') and isinstance(obj, type) and obj.__module__ == mod.__name__),
                None,
            )
            if mixin_cls is not None:
                found.append((stem, mixin_cls))
        except Exception:
            # 변형 빌드에서 파일이 없거나 의존성이 없는 경우 — 그냥 무시
            continue
    return found
